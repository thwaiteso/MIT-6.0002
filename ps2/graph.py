...
class WeightedEdge(Edge):
    def __init__(self, src, dest, total_distance, outdoor_distance):
        Edge.__init__(self, src, dest) # inherit from edge parent class
...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if node in self.nodes: # check if node already in dictionary
            raise ValueError('Duplicate node')
        else: # if node not in dictionary
            self.nodes.add(node)
            self.edges[node] = [] # add a new key to dictionary and assign it
            # an empty list
    ...
    def add_edge(self, edge):
        ...
        in the graph."""
        src = edge.get_source() # get source
        dest = edge.get_destination() # get destination
        total = edge.get_total_distance() # get total distance
        outdoor = edge.get_outdoor_distance() # get outdoor distance
        Weighted_edge = WeightedEdge(src, dest, total, outdoor)
        if not (src in self.nodes and dest in self.nodes): # check both source
            # and destination are in the dictionary i.e already added to graph
            raise ValueError('Node not in graph')
        self.edges[src].append(Weighted_edge) # add the destination (value) to 
        # the source (key)
