...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        # store information about floor tiles as a dictionary, with the key
        # being the position and the value being the dirt_amount
        self.tiles = {}
        for x in range(self.width):
            for y in range(self.height):
                self.tiles[(x, y)] = dirt_amount # dirt_amount is value at the
                # key (coordinates)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        # math.floor rounds floats to whole numbers
        x = math.floor(pos.get_x())
        y = math.floor(pos.get_y())
        self.tiles[x, y] -= capacity # take away the amount of dirt to be 
        # cleaned in a single time-step from the amount of dirt already on the
        # tile. If capacity is negative, dirt is added.
        if self.tiles[x, y] < 0: # if the amount of dirt left is less than 0, 
            # then capacity must have been greater than the amount of dirt on
            # the tile
            self.tiles[x, y] = 0 # therefore mark as 0
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        if self.tiles[(m, n)] == 0: # if tile is clean
            return True
        else: # if tile not clean
            return False
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        cleanTiles = 0
        for i in self.tiles: # iterate over keys in tiles dictionary
            if self.tiles[i] == 0: # if the value at a key is 0
                cleanTiles += 1 # add 1 to number of clean tiles
        return cleanTiles
    ...
    def is_position_in_room(self, pos):
        ...
        x = math.floor(pos.get_x())
        y = math.floor(pos.get_y())
        if (x, y) in self.tiles.keys(): # if the x,y coordinates are in the
            # dictionary
            return True
        else: # if not
            return False
    ...
    def get_dirt_amount(self, m, n):
        ...
        Returns: an integer
        """
        return self.tiles[(m, n)] # find the tile and return the value
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        """
        self.room = room
        self.speed = float(speed) # make a float
        self.capacity = capacity
        self.direction = float(random.uniform(0, 360)) # choose a random number
        # between 0 and 360 and make it a float
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        Returns: an integer; the total number of tiles in the room
        """
        return self.width * self.height # return area of room
    ...
    def get_random_position(self):
        ...
        # random numbers generated for width and height, were width and height
        # are the maximum numbers that can be returned to ensure a valid
        # position
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_tile_furnished(self, m, n):
        ...
        """
        if (m, n) in self.furniture_tiles: # if tile is valid
            return True
        else: # if tile not valid
            return False
    ...
    def is_position_furnished(self, pos):
        ...
        x = math.floor(pos.get_x())
        y = math.floor(pos.get_y())
        if (x, y) in self.furniture_tiles: # if tile is furnished
            return True
        else: # if tile not furnished
            return False
    ...
    def is_position_valid(self, pos):
        ...
        """
        if self.is_position_in_room(pos) and not \
        self.is_position_furnished(pos): # if position is in the room and is
            # unfurnished
            return True
        else: # if position is not in the room and/or is furnished
            return False
    ...
    def get_num_tiles(self):
        ...
        """
        return len(self.tiles) - len(self.furniture_tiles) # subtract furnished
        # tiles from total number of tiles
    ...
    def get_random_position(self):
        ...
        pos = Position(random.uniform(0, self.width), 
                        random.uniform(0, self.height)) # random valid x and y
        while self.is_position_furnished(pos): # if pos is furnished
            pos = Position(random.uniform(0, self.width), 
                        random.uniform(0, self.height)) # new random pos
        if self.is_position_valid(pos): # if pos is valid
            return pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        # get new position
        if self.room.is_position_valid(newPos): # if newPos is valid
            self.set_robot_position(newPos) # set to new position
            self.room.clean_tile_at_position(newPos, self.capacity) # clean tile
        else: # if newPos not valid
            self.set_robot_direction(random.uniform(0, 360)) # change direction
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        newPos = self.position.get_new_position(self.direction, self.speed) 
        # get new position
        if not self.gets_faulty() and self.room.is_position_valid(newPos): 
            # if the robot is not faulty and the position in the room is valid
            self.set_robot_position(newPos) # set robot to  position
            self.room.clean_tile_at_position(newPos, self.capacity) # clean 
            # tile at position by capacity amount
        else: # if the robot is  faulty and/or the position in the room is 
            # not valid
            self.set_robot_direction(random.uniform(0, 360)) # pick a new
            # direction
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    allSteps = [] # list to track number of steps for each trial
    
    for trial in range(num_trials): # run a trial by amount specified in 
        # num_trials
        room = EmptyRoom(width, height, dirt_amount) # set up room (only empty
        # room will be used)
        robots = [] # list to track number of robots
        for robot in range(num_robots): # make robots by amount specifed in
            # num_robots
            robots.append(robot_type(room, speed, capacity)) # add robots to
            # list
        timeSteps = 0 # number of steps in this trial
        while (room.get_num_cleaned_tiles() / room.get_num_tiles()) <= \
        min_coverage: # while the proportion of clean tiles is less than or 
            # equal to the minimum coverage required
           for robot in robots: # for each robot
               robot.update_position_and_clean() # change position and clean
           timeSteps += 1 # add one to steps
        allSteps.append(timeSteps) # when done, add number of steps in this 
        # trial to allSteps
    return (sum(allSteps) / num_trials) # return the sum of steps in all the 
    # trials divided by the number of trials (mean steps per trial)
...
#       of a 20x20 room?
#   StandardRobot is quicker across any number of robots, though not by much
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#   StandardRobot is quicker across any size room, with FaultyRobot getting
#   increasingly worse with larger rooms
